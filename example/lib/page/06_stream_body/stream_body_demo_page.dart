...
class StreamBodyDemoPage extends StatefulWidget {
  @override
  _StreamBodyDemoPageState createState() => _StreamBodyDemoPageState();
}

class _StreamBodyDemoPageState extends State<StreamBodyDemoPage> {
  StreamController<List<List<Shop>>> _dataController = StreamController();
  ...
  Widget _buildBody() {
    return StoneStreamBody<List<List<Shop>>>(
      onRetry: _fetchData,
  ...
  void _fetchData([StoneFetchDataError error]) {
    ...
    _dataController.addStream(_DataProvider.fetchShopGroups(error).asStream());
  }
}

class _DataProvider {
  static Future<List<List<Shop>>> fetchShopGroups([StoneFetchDataError error]) async {
